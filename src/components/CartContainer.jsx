...
function CartContainer() {
  ...
  useEffect(() => {
    ...
    setTot(totalPrice); // Cập nhật biến tot với giá trị totalPrice
    setFlag(true); // Cập nhật biến flag thành true
  }, [cartItems]); // Sử dụng cartItems để theo dõi sự thay đổi
  {
    cartItems &&
      cartItems.map((item) => (
        ...
        justify-evenly px-8 py-2"
          >
            <div className="w-full flex items-center justify-between">
              ...
              <p className="text-gray-400 text-lg"> {tot} VNĐ</p>
            </div>
            <div className="w-full flex items-center justify-between">
              <p className="text-gray-400 text-lg">Phí giao hàng</p>
              <p className="text-gray-400 text-lg"> 8.000 VNĐ</p>
            </div>
